/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { SoundsLabelsCreateNestedManyWithoutLabelsItemsInput } from "./SoundsLabelsCreateNestedManyWithoutLabelsItemsInput";
import { Type } from "class-transformer";
import { TextsLabelsCreateNestedManyWithoutLabelsItemsInput } from "./TextsLabelsCreateNestedManyWithoutLabelsItemsInput";

@InputType()
class LabelsCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => SoundsLabelsCreateNestedManyWithoutLabelsItemsInput,
  })
  @ValidateNested()
  @Type(() => SoundsLabelsCreateNestedManyWithoutLabelsItemsInput)
  @IsOptional()
  @Field(() => SoundsLabelsCreateNestedManyWithoutLabelsItemsInput, {
    nullable: true,
  })
  soundsLabelsItems?: SoundsLabelsCreateNestedManyWithoutLabelsItemsInput;

  @ApiProperty({
    required: false,
    type: () => TextsLabelsCreateNestedManyWithoutLabelsItemsInput,
  })
  @ValidateNested()
  @Type(() => TextsLabelsCreateNestedManyWithoutLabelsItemsInput)
  @IsOptional()
  @Field(() => TextsLabelsCreateNestedManyWithoutLabelsItemsInput, {
    nullable: true,
  })
  textsLabelsItems?: TextsLabelsCreateNestedManyWithoutLabelsItemsInput;
}

export { LabelsCreateInput as LabelsCreateInput };
