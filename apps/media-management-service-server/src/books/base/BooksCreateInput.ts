/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AccountsBooksCreateNestedManyWithoutBooksItemsInput } from "./AccountsBooksCreateNestedManyWithoutBooksItemsInput";
import {
  ValidateNested,
  IsOptional,
  IsString,
  MaxLength,
} from "class-validator";
import { Type } from "class-transformer";
import { ClientsBooksCreateNestedManyWithoutBooksItemsInput } from "./ClientsBooksCreateNestedManyWithoutBooksItemsInput";
import { TextsCreateNestedManyWithoutBooksItemsInput } from "./TextsCreateNestedManyWithoutBooksItemsInput";

@InputType()
class BooksCreateInput {
  @ApiProperty({
    required: false,
    type: () => AccountsBooksCreateNestedManyWithoutBooksItemsInput,
  })
  @ValidateNested()
  @Type(() => AccountsBooksCreateNestedManyWithoutBooksItemsInput)
  @IsOptional()
  @Field(() => AccountsBooksCreateNestedManyWithoutBooksItemsInput, {
    nullable: true,
  })
  accountsBooksItems?: AccountsBooksCreateNestedManyWithoutBooksItemsInput;

  @ApiProperty({
    required: false,
    type: () => ClientsBooksCreateNestedManyWithoutBooksItemsInput,
  })
  @ValidateNested()
  @Type(() => ClientsBooksCreateNestedManyWithoutBooksItemsInput)
  @IsOptional()
  @Field(() => ClientsBooksCreateNestedManyWithoutBooksItemsInput, {
    nullable: true,
  })
  clientsBooksItems?: ClientsBooksCreateNestedManyWithoutBooksItemsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => TextsCreateNestedManyWithoutBooksItemsInput,
  })
  @ValidateNested()
  @Type(() => TextsCreateNestedManyWithoutBooksItemsInput)
  @IsOptional()
  @Field(() => TextsCreateNestedManyWithoutBooksItemsInput, {
    nullable: true,
  })
  textsItems?: TextsCreateNestedManyWithoutBooksItemsInput;
}

export { BooksCreateInput as BooksCreateInput };
