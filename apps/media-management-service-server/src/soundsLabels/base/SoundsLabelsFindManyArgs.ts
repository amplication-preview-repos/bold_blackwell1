/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SoundsLabelsWhereInput } from "./SoundsLabelsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { SoundsLabelsOrderByInput } from "./SoundsLabelsOrderByInput";

@ArgsType()
class SoundsLabelsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => SoundsLabelsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => SoundsLabelsWhereInput, { nullable: true })
  @Type(() => SoundsLabelsWhereInput)
  where?: SoundsLabelsWhereInput;

  @ApiProperty({
    required: false,
    type: [SoundsLabelsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [SoundsLabelsOrderByInput], { nullable: true })
  @Type(() => SoundsLabelsOrderByInput)
  orderBy?: Array<SoundsLabelsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { SoundsLabelsFindManyArgs as SoundsLabelsFindManyArgs };
